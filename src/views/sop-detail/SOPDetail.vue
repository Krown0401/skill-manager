    ...
    <el-card style="height: 100%">
      <template #header>
        ...
        >
          <div>
            <el-input
              :model-value="sop?.name"
              @update:model-value="
                (v) => {
                  if (sop.value) sop.value.name = v;
                }
              "
              size="default"
            ...
            <el-tag
              v-if="sop?.source"
              :type="sop.source === 'generated' ? 'success' : 'warning'"
              size="small"
      ...
      <el-row :gutter="12" style="height: 100%">
        <el-col :span="17" style="height: 100%">
          <div style="margin-bottom: 8px">
            <el-input
              :model-value="sop?.goal"
              @update:model-value="
                (v) => {
                  if (sop.value) sop.value.goal = v;
                }
              "
              type="textarea"
              :rows="2"
              placeholder="SOP 目标描述..."
            />
          </div>
          <div
            style="
        ...
        <el-col :span="7" style="height: 100%; overflow-y: auto">
          <div v-if="selectedNode" style="margin-bottom: 12px">
            <el-descriptions :column="1" border size="default" title="节点详情">
              ...
              <el-descriptions-item
                v-if="selectedNode.skill_id"
                label="关联 Skill"
              >
                <el-tag type="primary" size="small">{{
                  skillName(selectedNode.skill_id)
                }}</el-tag>
              </el-descriptions-item>
              <el-descriptions-item
                v-if="selectedNode.condition_expr"
                label="条件表达式"
              >
                <code style="word-break: break-all">{{
                  selectedNode.condition_expr
                }}</code>
              </el-descriptions-item>
              <el-descriptions-item
                v-if="selectedNode.manual_checklist?.length"
                label="人工确认项"
              >
                <ul style="margin: 0; padding-left: 18px">
                  <li
                    v-for="(c, i) in selectedNode.manual_checklist"
                    :key="i"
          ...
          <el-timeline v-if="sop?.success_criteria?.length">
            <el-timeline-item
              v-for="(c, i) in sop.success_criteria"
              :key="i"
              type="primary"
              >{{ c }}</el-timeline-item
            >
          ...
          <el-empty
            v-else
            description="暂无成功标准，可在编辑中补充"
            :image-size="60"
          />

          <el-divider content-position="left" v-if="sop?.explanation"
            >编排说明</el-divider
          >
          <el-alert
            v-if="sop?.explanation"
            type="info"
            :closable="false"
            show-icon
          >
